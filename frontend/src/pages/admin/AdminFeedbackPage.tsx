...
export default function AdminFeedbackPage() {
	...
	const handleApproveFeedback = async (id: string) => {
		try {
			...
			// Update local state
			setAllFeedback((prev) =>
				prev.map((f) => (f.id === id ? { ...f, isApproved: true } : f)),
			);
		} catch (error) {
	...
	return (
		<div className="space-y-6">
			...
			<Tabs
				value={activeTab}
				onValueChange={(v) => setActiveTab(v as "all" | "pending" | "approved")}
			>
				<div className="flex justify-between items-center mb-4">
					<TabsList>
						<TabsTrigger value="all">All ({allFeedback.length})</TabsTrigger>
						<TabsTrigger value="pending">
							Pending ({allFeedback.filter((f) => !f.isApproved).length})
						</TabsTrigger>
						<TabsTrigger value="approved">
							Approved ({allFeedback.filter((f) => f.isApproved).length})
						</TabsTrigger>
					</TabsList>
