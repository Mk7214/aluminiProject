...
export default function ScraperPage() {
	...
	const handleScrape = async () => {
		...
		try {
			...
			setError(
				error instanceof Error ? error.message : "Failed to scrape profiles",
			);
			toast.error(
				error instanceof Error ? error.message : "Failed to scrape profiles",
			);
	...
	const handleAddAlumni = async (profile: ScrapedProfile) => {
		try {
			...
			const response = await fetch(
				"http://localhost:5000/api/scraper/add-scraped-profile",
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			if (!response.ok) {
				...
				if (
					...
					setScrapedProfiles((prev) =>
						prev.filter((p) => p.email !== profile.email),
					);
					return;
				}
			...
			const errorMessage =
				error instanceof Error ? error.message : "Failed to add alumni";
			toast.error(errorMessage);

			// If there are specific validation errors, show them
			if (
				error instanceof Error &&
				(error as { cause?: { missingFields?: string[] } }).cause &&
				typeof (error as { cause?: { missingFields?: string[] } }).cause ===
					"object"
			) {
				const cause = (error as { cause?: { missingFields?: string[] } }).cause;
				if (cause?.missingFields) {
					toast.error(
						`Missing required fields: ${cause.missingFields.join(", ")}`,
					);
				}
			}
		}
				...
				<TabsContent value="search">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Search for potential alumni on LinkedIn and Naukri.com using
								keywords.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="space-y-2">
									...
									<Select
										value={source}
										onValueChange={(value: "linkedin" | "naukri") =>
											setSource(value)
										}
									>
				...
				<TabsContent value="results">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								{scrapedProfiles.length} profiles found.{" "}
								{meta &&
									`Searched ${meta.source} with ${meta.keywords.join(", ")} (${meta.count} total results).`}{" "}
								Add them as alumni to your database.
							</CardDescription>
						</CardHeader>
						<CardContent>
							{error ? (
								...
								<div className="text-center py-8">
									<p className="text-gray-500 mb-4">
										No profiles found. Try a new search.
									</p>
									<Button
										variant="outline"
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
									{scrapedProfiles.map((profile) => (
										<Card
											...
											style={{
												borderLeftColor:
													profile.source === "LinkedIn" ? "#0077b5" : "#FF6200",
											}}
										>
