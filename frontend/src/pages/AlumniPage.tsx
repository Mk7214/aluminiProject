	...
	useEffect(() => {
		const fetchAlumni = async () => {
			try {
				...
				const transformedAlumni = alumniList
					.map((a) => {
				...
				const uniqueBatches = [
					...new Set(transformedAlumni.map((a) => a.batch).filter(Boolean)),
				]
					.sort()
					.reverse();
				const uniqueDegrees = [
					...new Set(transformedAlumni.map((a) => a.degree).filter(Boolean)),
				].sort();
				const uniqueCompanies = [
					...new Set(
						transformedAlumni
							.filter(
								(a) => a.currentCompany && typeof a.currentCompany === "string",
							)
